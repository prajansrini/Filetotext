...
(function () {
  ...
  document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') {
      if (imgModal && imgModal.classList.contains('active')) imgModal.classList.remove('active');
      if (infoModal && infoModal.classList.contains('active')) infoModal.classList.remove('active');
      const bm = $('#batchModal'); if (bm && bm.classList.contains('active')) bm.classList.remove('active');
      const pm = $('#promptModal'); if (pm && pm.classList.contains('active')) { const pc = $('#promptCancel'); if (pc) pc.click(); }
    }
  });
  ...
  if (infoBtn && infoModal && infoCloseBtn) {
    ...
    infoCloseBtn.onclick = () => infoModal.classList.remove('active');
  ...
  function customPrompt(title, btnText = 'OK', showInput = true) {
    return new Promise((resolve) => {
      ...
      btnOk.onclick = () => { cleanup(); resolve(showInput ? inputEl.value : true); };
      inputEl.onkeydown = (e) => {
        if (e.key === 'Enter') { cleanup(); resolve(showInput ? inputEl.value : true); }
  ...
  function loadWorkspace() {
    ...
    txEnc.objectStore('encodeFiles').getAll().onsuccess = (e) => {
      ...
      for (const rec of e.target.result) {
        ...
        if (rec.type.startsWith('image/') && !genericImages.has(rec.type)) addImg(rec.file, true, rec.id);
  ...
  if (logoIcon) {
    ...
    function spinLoop() {
      if (!isDrag) velocity = velocity * friction + idleSpeed * (1 - friction);
      curRot += velocity;
      logoIcon.style.transform = `rotate(${curRot}deg)`;
  ...
  async function compressBytes(bytes) {
    ...
    const readPromise = readStreamFully(cs.readable);
    const writer = cs.writable.getWriter();
    const chunk = 10 * 1024 * 1024;
    for (let i = 0; i < bytes.length; i += chunk) { await writer.write(bytes.subarray(i, i + chunk)); await sleep(0); }
    await writer.close();
    return await readPromise;
  }
  async function decompressBytes(bytes) {
    const ds = new DecompressionStream('deflate');
    const readPromise = readStreamFully(ds.readable);
  ...
  async function gzipJSON(jsonStr) {
    ...
    const writer = cs.writable.getWriter();
    const chunk = 10 * 1024 * 1024;
    for (let i = 0; i < enc.length; i += chunk) { await writer.write(enc.subarray(i, i + chunk)); await sleep(0); }
    await writer.close();
    return await readPromise;
  }
  async function ungzipJSON(u8) {
    ...
    const writer = ds.writable.getWriter();
    const chunk = 10 * 1024 * 1024;
    for (let i = 0; i < u8.length; i += chunk) { await writer.write(u8.subarray(i, i + chunk)); await sleep(0); }
    await writer.close();
  ...
  function makeImgRow(id, file, imgEl, dataUrl) {
    ...
    row.innerHTML =
      ...
      '<div class="option-group split-mob"><label class="option-label">Original size</label><label class="toggle-switch"><input type="checkbox" class="js-orig" checked><span class="toggle-slider"></span></label></div>' +
      '<div class="option-group split-mob"><label class="option-label">Max Dimensions</label><input type="number" class="option-input js-maxdim" value="256" min="1" max="4096" disabled></div>' +
      '</div>' +
      ...
      '<div class="option-group split-mob"><label class="option-label">Encrypt</label><label class="toggle-switch"><input type="checkbox" class="js-enc-tog"><span class="toggle-slider"></span></label></div>' +
      '<div class="option-group split-mob"><label class="option-label">Password</label><input type="password" class="option-input js-enc-pass" placeholder="Secret..." disabled></div>' +
      '</div>' +
      '<div class="row-result-header" style="border-top: 1px solid var(--border); padding-top: 10px; margin-top: 10px;">' +
      '<h4>File Preview</h4>' +
      '<div class="card-actions">' +
      '<button class="btn btn-sm btn-accent js-dl-orig">Download File</button>' +
      '</div></div>' +
      '<div class="generic-preview-wrap" style="border-top:none; margin-top: 10px; text-align: center; overflow: hidden; background: var(--bg-input); border-radius: 6px;">' +
      '<img src="' + dataUrl + '" class="js-img-preview" style="max-width: 100%; max-height: 400px; border-radius: 6px; object-fit: contain; cursor: zoom-in;" alt="">' +
      '</div>' +
    ...
    maxIn.onchange = () => {
      let v = parseInt(maxIn.value);
      if (isNaN(v) || v < 1) maxIn.value = 1;
      else if (v > 4096) maxIn.value = 4096;
    };
    ...
    const imgPrev = row.querySelector('.js-img-preview');
    if (imgPrev) {
      imgPrev.onclick = () => openImagePreview(dataUrl);
    }

    const dlOrigBtn = row.querySelector('.js-dl-orig');
    if (dlOrigBtn) {
      dlOrigBtn.onclick = () => {
        const origFile = row._origFile;
  ...
  function makeGenericRow(id, file, u8) {
    ...
    row.innerHTML =
      ...
      '<button class="btn btn-sm btn-accent js-dl-orig">Download File</button>' +
      '</div></div>' +
      '<div class="generic-preview-wrap" style="border-top:none; margin-top: 10px;">' + previewHtml + '</div>' +
      '<div class="row-progress"><div class="row-progress-bar"></div></div>' +
      '<div class="row-result" id="res-' + id + '">' +
      '<div class="row-result-header"><h4>Encoded Text Preview</h4><div class="card-actions">' +
      '<span class="badge badge-enc js-eb"></span><span class="badge badge-size js-js"></span>' +
    ...
    const fmtSel = row.querySelector('.js-fmt'), encSel = row.querySelector('.js-enc');
    const updateEncOpts = () => {
      const isPxsn2 = fmtSel.value === 'pxsn2';
      const oldVal = encSel.value;
      encSel.innerHTML = isPxsn2
        ? '<option value="gzip">GZip (Raw Bytes)</option><option value="base64">Base64 Text</option><option value="base85">Base85 Text</option>'
        : '<option value="base64">Base64 Text</option><option value="base85">Base85 Text</option>';
      if (isPxsn2 && oldVal) encSel.value = oldVal;
      else if (!isPxsn2 && oldVal && oldVal !== 'gzip') encSel.value = oldVal;
    };
    fmtSel.onchange = updateEncOpts;
    updateEncOpts();
  ...
  async function convertGenericRow(row, id) {
    ...
    showProgress(row);
    await sleep(50); // Give UI more time to breathe and start animation

    let compressed;
    if (outFmt !== 'pxsn2') {
      if (!row._u8) row._u8 = new Uint8Array(await row._origFile.arrayBuffer());
      compressed = await compressBytes(row._u8);
    }
    ...
    if (outFmt !== 'pxsn2') {
      ...
      encLabel = enc === 'gzip' ? 'GZip (Raw Bytes)' : enc;
      encData = ""; // Not used for pxsn2
    }

    const origSize = row._u8 ? row._u8.length : row._fileSize;
    const obj = { format: 'pixson-file-v1', filename: row._fn, mimeType: row._mime, originalSize: origSize, encoding: encLabel, data: encData };
    row._jd = obj;
    const jsonStr = outFmt !== 'pxsn2' ? JSON.stringify(obj, null, 2) : '';
    ...
    if (outFmt === 'pxsn2') {
      ...
      const bin = createGenericPxnBinary(row._fn, row._mime, origSize, compressed);
      fileBlob = new Blob([bin], { type: 'application/octet-stream' });
      fileExt = 'pxsn';
      displaySize = bin.length;
      previewStr = hexDump(bin);
    ...
    const fmtEl = row.querySelector('.js-fmt');
    const encEl = row.querySelector('.js-enc');
    let containerName = fmtEl.options[fmtEl.selectedIndex].text.split(' (')[0];
    let encodingName = encEl.options[encEl.selectedIndex].text.split(' (')[0];
    let displayEnc = containerName + ' + ' + encodingName;
    if (row.querySelector('.js-enc-tog').checked) displayEnc += ' 🔒 (AES-256)';
    ...
    toast(fmtSz(origSize) + ' → ' + fmtSz(displaySize) + ' (' + ((displaySize / origSize) * 100).toFixed(0) + '%)', 'success');
  }
  ...
  async function convertImgRow(row, id) {
    ...
    let containerName = fmtEl.options[fmtEl.selectedIndex].text.split(' (')[0];
    let encodingName = encEl.options[encEl.selectedIndex].text.split(' (')[0];
    displayEnc = containerName + ' + ' + encodingName;
    if (encTog) displayEnc += ' 🔒 (AES-256)';
  ...
  document.onpaste = e => {
    ...
    for (const it of items) if (it.type.startsWith('image/')) { 
      ...
      if (!b) return;
      if (genericImages.has(it.type)) addGenericFile(new File([b], 'pasted-' + Date.now() + (it.type.includes('svg') ? '.svg' : it.type.includes('gif') ? '.gif' : '.bmp'), { type: it.type }));
      else addImgBlob(b); 
      return; 
    }
  };
  ...
  async function reconRow(row, id) {
    ...
    if (enc === 'webp' && obj.data) {
      ...
      if ((enc === 'base64' || enc === 'pixen' || enc === 'pixenultra') && (obj.data || obj._binaryData)) {
        ...
        for (let y = 0; y < h; y++) {
          const r = obj.pixels[y]; if (!Array.isArray(r)) { toast('Row ' + y + ' invalid', 'error'); btn.disabled = false; btn.textContent = 'Reconstruct'; return; }
  ...
  window.updateBulkUI = function () {
    ...
    if (encRows.length > 0) {
      ...
      $('#btnDownloadAllEncode').disabled = false;
      const bundleBtn = $('#btnBundleAllEncode'); if (bundleBtn) bundleBtn.disabled = false;
      $('#btnProcessAllEncode').disabled = false;
  ...
  $('#btnClearAllEncode').onclick = async () => { if (await customPrompt('Clear all files? This cannot be undone.', 'Clear', false)) { imgQueue.innerHTML = ''; clearDBStore('encodeFiles'); } };
  $('#btnClearAllDecode').onclick = async () => { if (await customPrompt('Clear all files? This cannot be undone.', 'Clear', false)) { jsonQueue.innerHTML = ''; clearDBStore('decodeFiles'); } };
  ...
  function openBatchModal(action, isEncode, batchId = null) {
    ...
    else if (action === 'download') actionText = 'Download ZIP';
    else if (action === 'bundle') actionText = 'Bundle into 1 File';
    else if (action === 'clear') actionText = 'Clear Selected';

    $('#batchModalTitle').textContent = actionText;

    const listEl = $('#batchModalList');
    listEl.innerHTML = '';

    rows.forEach(row => {
      let metaHtml = '';
      if (currentBatchAction === 'clear' || currentBatchAction === 'download') {
         // No extra meta UI needed for clear or download (download has its own section below)
      } else if (isEncode) {
        const fmt = row.querySelector('.js-fmt');
        const enc = row.querySelector('.js-enc');
        metaHtml = `
          <select class="batch-fmt-sel option-select" style="padding:2px 5px;font-size:0.75rem;margin-right:5px;width:auto;" data-row-id="${row.id}">${fmt.innerHTML}</select>
          <select class="batch-enc-sel option-select" style="padding:2px 5px;font-size:0.75rem;width:auto;" data-row-id="${row.id}">${enc.innerHTML}</select>
        `;
      } else {
        metaHtml = row._obj ? (row._obj.encoding || 'Auto') : 'Auto';
      }

      const isSel = row[`_batchSel_${currentBatchAction}`] !== false;
      const isOrig = row[`_batchOrig_${currentBatchAction}`] !== false;
      const isEnc = row[`_batchEnc_${currentBatchAction}`] !== false;
      ...
      item.innerHTML = `
        <div style="display:flex;align-items:center;margin-bottom:8px;">
          ...
          <div style="font-size:.88rem;font-weight:600;flex-grow:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="${row._fn || row._name}">${row._fn || row._name || 'File'}</div>
        </div>
    ...
    allCbs.forEach((cb, idx) => {
      ...
      const mFmt = itemEl.querySelector('.batch-fmt-sel');
      ...
      mFmt.onchange = () => {
        rFmt.value = mFmt.value;
        if (rFmt.onchange) rFmt.onchange({ target: rFmt });
        else rFmt.dispatchEvent(new Event('change'));
        mEnc.innerHTML = rEnc.innerHTML;
        mEnc.value = rEnc.value;
      };
      mEnc.onchange = () => {
        rEnc.value = mEnc.value;
        if (rEnc.onchange) rEnc.onchange({ target: rEnc });
        else rEnc.dispatchEvent(new Event('change'));
      };
    });

    const bsa = $('#batchSelectAll'); if (bsa) bsa.checked = allChecked;
  ...
  if (bsa) {
    bsa.onchange = (e) => {
      Array.from(document.querySelectorAll('.batch-file-cb')).forEach(cb => {
  ...
  async function autoCompileBulk(btn, activeText, selectedRows) {
  ...
  $('#btnProcessAllEncode').onclick = () => openBatchModal('encode', true);
  ...
  if (bmp) {
    bmp.onclick = async () => {
      $('#batchModal').classList.remove('active');
      const checkedIds = Array.from(document.querySelectorAll('.batch-file-cb:checked')).map(cb => cb.dataset.rowId);
      if (!checkedIds.length) { toast('No files selected', 'warning'); return; }

      const queue = currentBatchIsEncode ? imgQueue : jsonQueue;
      const rows = checkedIds.map(id => queue.querySelector('#' + id)).filter(Boolean);

      if (currentBatchAction === 'clear') {
         rows.forEach(row => {
            const rmBtn = row.querySelector('.js-rm');
            if (rmBtn) rmBtn.click();
         });
         return;
      }

      if (currentBatchIsEncode) {
        if (currentBatchAction === 'encode') {
          const btn = $('#btnProcessAllEncode');
          const origText = btn.textContent;
          await autoCompileBulk(btn, 'Processing...', rows);
          btn.disabled = false; btn.textContent = origText;
        } else if (currentBatchAction === 'bundle') {
          const btn = $('#btnBundleAllEncode');
          const origText = btn.textContent;
          const allRows = await autoCompileBulk(btn, 'Compiling...', rows);
          const readyRows = allRows.filter(r => r._dlBlob);
          if (!readyRows.length) { toast('No files successfully encoded to bundle.', 'error'); btn.disabled = false; btn.textContent = origText; return; }
          const pass = await customPrompt('Optional: Enter a password to encrypt this entire bundle', 'Bundle');
          if (pass === null) { btn.disabled = false; btn.textContent = origText; return; }
          btn.disabled = true; btn.textContent = 'Bundling...';

          try {
            const files = readyRows.map(r => r._origFile || (r._u8 ? new File([r._u8], r._fn, { type: r._mime }) : null)).filter(Boolean);
          ...
          for (const row of allRows) {
            ...
            if (!cbOrig && !cbEnc) {
              if (row._dlBlob) {
                const buf = await row._dlBlob.arrayBuffer(); files.push({ name: row._dlName, u8: new Uint8Array(buf) });
              }
            } else {
              if (cbOrig && cbOrig.checked) {
                const origFile = row._origFile || new File([row._u8], row._fn, { type: row._mime });
                const buf = await origFile.arrayBuffer();
                files.push({ name: 'original_' + row._fn, u8: new Uint8Array(buf) });
              }
              if (cbEnc && cbEnc.checked && row._dlBlob) {
                const buf = await row._dlBlob.arrayBuffer(); files.push({ name: row._dlName, u8: new Uint8Array(buf) });
              }
            }
          }
        ...
        if (currentBatchAction === 'encode') {
          ...
          const btn = $('#btnDownloadAllDecode');
  ...
  const wittyQuotes = [
    "Gone binary. Back in a bit (literally).",
    "Your file's undercover identity: plain text.",
    "Just a humble wall of characters. Nothing to see here.",
    "Like ZIP, but with an identity crisis.",
    "Your MP3 is now doing its English essay.",
    "Because sometimes a file needs to cosplay as text.",
    "Files in disguise. More than meets the eye.",
    "A file walks into a text field. The bouncer doesn't notice.",
    "Smuggling files through plain text since 2026.",
    "Your file's undercover identity: plain text.",
    "If Base64 had a glow-up.",
    "The witness protection program for files.",
    "Your file called. It wants to be a string when it grows up.",
    "Ctrl+C for files that refuse to be copied.",
    "This totally normal text blob is definitely not a ZIP file.",
    "If Base64 had a glow-up.",
    "PDF? Never heard of it. This is just... words.",
    "The greatest disguise since sunglasses and a fake mustache.",
    "0s and 1s in a text costume.",
    "When email says \"attachments not allowed.\"",
    "Text: the universal skeleton key.",
    "Your binary's gap year: becoming a string.",
    "It's giving... file in a trench coat pretending to be text.",
    "A DOCX in sheep's clothing.",
    "The internet was built on text anyway.",
    "Making attachments out of nothing at all... just text.",
    "Because not every platform deserves your .pdf.",
    "Compression: because size matters.",
    "When your file needs a fake ID to get past the firewall.",
    "Sneak your files through text-only doors.",
    "Your file's villain arc: pretending to be plaintext.",
    "Base64 is a lifestyle, not just an encoding.",
    "Looks like text. Secretly isn't.",
    "Not all heroes wear capes. Some wear quotation marks.",
    "Turning \"you can't send that\" into \"watch me.\"",
    "What your attachment looks like after a disguise montage.",
    "The Swiss Army knife of copy and paste.",
    "Files that lie about their MIME type.",
    "For the moments when only text gets past security.",
    "This text has a secret. It used to be a spreadsheet.",
    "ASCII would be proud.",
    "When the upload button isn't an option, become the text.",
    "Definitely not a PNG wearing a fake mustache.",
    "Turning forbidden attachments into boring text since 2026.",
    "Your PDF identifies as a paragraph now.",
    "The art of smuggling bytes through a textarea.",
  ...
  // ─── Exports for PXSN v2 ─────────────────────────────────────
  window.pixenEncode = async function (rgba, w, h) {
    const rawRGB = new Uint8Array(w * h * 3);
    for (let i = 0, j = 0; i < rgba.length; i += 4, j += 3) {
      rawRGB[j] = rgba[i]; rawRGB[j + 1] = rgba[i + 1]; rawRGB[j + 2] = rgba[i + 2];
    }
    const filtered = applyBestFilter(rawRGB, w);
    return await compressBytes(filtered);
  };

  window.pixenUltraEncode = async function (rgba, w, h) {
    const rawRGB = new Uint8Array(w * h * 3);
    for (let i = 0, j = 0; i < rgba.length; i += 4, j += 3) {
      rawRGB[j] = rgba[i]; rawRGB[j + 1] = rgba[i + 1]; rawRGB[j + 2] = rgba[i + 2];
    }
  ...
  window.pixenUltraDecode = async function (payload, w, h) {
    const packed = await decompressBytes(payload);
    const rawRGB = unpackPixenUltra(packed);
    const rgba = new Uint8ClampedArray(w * h * 4);
    for (let i = 0, j = 0; j < rawRGB.length; i += 4, j += 3) {
      rgba[i] = rawRGB[j]; rgba[i + 1] = rawRGB[j + 1]; rgba[i + 2] = rawRGB[j + 2]; rgba[i + 3] = 255;
    }
